...
class retraitescreen extends StatefulWidget {
  const retraitescreen({Key? key}) : super(key: key);
...
class RetraitescreenState extends State<retraitescreen>
{
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      //backgroundColor: Color(0XFFe3dfde),
      appBar: AppBar(
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              SizedBox(height: 10),
              Text('Quelle est votre date de Naissance? (veuillez respecter les format jour/mois/année)'),SizedBox(height: 5),
              TextField(
                controller: ddnCtrl,
              ...
              SizedBox(height: 10),
              Text('A quel régime cotisez-vous ?',),SizedBox(height: 5),
              Container(
                ...
                child: DropdownButton<String>(
                  ...
                  items: <String>['RG', 'FCT', 'FNP', 'DMG',].map<DropdownMenuItem<String>>((String value) {
              ...
              SizedBox(height: 10),
              Text("Pendant combien d'années avez-vous travaillé entre 1976 et 2001"),SizedBox(height: 5),
              TextField(
              ...
              SizedBox(height: 10),
              Text("Pendant combien d'années avez-vous travaillé entre 2002 et 2007"),SizedBox(height: 5),
              TextField(
              ...
              SizedBox(height: 10),
              Text("Pendant combien d'années avez-vous travaillé de 2008 à aujourd'hui"),SizedBox(height: 05),
              TextField(
              ...
              SizedBox(height: 10),
              Text("Indiquez la moyenne de votre salaire brut mensuel des 10 dernères années"),SizedBox(height: 5),
              TextField(
              ...
              afficher_label_indice_solde ? Text('Indice du solde') : Text(""),
              afficher_et_indice_solde ? TextField(
              ...
              ElevatedButton(
                  onPressed: (){
                    int nbreAnnee1 = nbreAnneeCtrl.text.isEmpty ? 0 : int.parse(nbreAnneeCtrl.text);
                    int nbreAnnee2 = nbreAnneeCtrl2.text.isEmpty ? 0 : int.parse(nbreAnneeCtrl2.text);
                    int nbreAnnee3 = nbreAnneeCtrl3.text.isEmpty ? 0 : int.parse(nbreAnneeCtrl3.text);

                    double salaireBrut = salaireCtrl.text.isEmpty ? 0.0 : double.parse(salaireCtrl.text);

                    int dureeCotisation = nbreAnnee1 + nbreAnnee2 + nbreAnnee3;

                    var dateNaissanceSplit = ddnCtrl.text.split('/');
                    if(dateNaissanceSplit.length < 3)
                      MAlertDialog.show("Format de la date incorrect", context);
                    else{
                      ...
                      if(_chosenValue == "RG")
                        {
                          MAlertDialog.show(
                          regimeRG(age, dureeCotisation, nbreAnnee1, nbreAnnee2, nbreAnnee3, salaireBrut).toString(),
                          context);
                        }
                    }
                  },
                  child: Container(
                    ...
                    width: double.infinity,
                    child: Center(child: Text("SIMULER"),),
                  )
      ...
      double salaireBrut)
  {
    ...
    } else if(dureeCotisation < 25)
    {
      ...
      pensionNet = (pensionBrute > 50000) ?  pensionBrute - (pensionBrute*0.03) : pensionBrute;
    }
    return pensionNet;
  }
}
